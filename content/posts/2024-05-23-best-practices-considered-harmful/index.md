...
slug: "/2024-05-23-best-practices-considered-harmful"
date: 2024-04-25
canonicalUrl: "https://elvischidera.com/2024-05-23-best-practices-considered-harmful"
...
The software industry has an extensive repertoire of so-called “best practices” that, in theory, should produce net positive results. Some of these practices are inherently useful but suffer in how they are disseminated, adopted, and applied. Meanwhile, some “best practices” have a negative ROI.

The raison d'être of this essay is to surface some of the perils of mindlessly using “best practices”. I chose the title for its comical effect in reference to Dijkstra’s infamous essay “[Goto considered harmful](https://homepages.cwi.nl/~storm/teaching/reader/Dijkstra68.pdf)” and the [history](https://meyerweb.com/eric/comment/chech.html) around it.
...
The relationship between two things is [seldom linear](https://www.lesswrong.com/posts/dgvTCGS5egCdZ6uJF/linearity-fallacies-1). There is such a thing as “too much of a good thing”. Pick any widespread useful practice, and I will show you how it has been overused and underutilized. I will expand on this point with test coverage in the following paragraphs.
...
I’m not against test coverage. I want teams to think from first principles before embarking on a month-long crusade to improve test coverage. Ask questions like: If we scale these tests, would we have detected any of the last 10 real issues we encountered? Have we peaked this strategy? Is this even an effective testing strategy (i.e., are we getting more confident in our code)?
...
> Psychological research suggests that people only maintain partial descriptions of the things to be remembered — [The Design of Everyday Things](https://elvischidera.com/2022-06-24-design-everyday-things)
 
Typically, the variant of an idea that has gained virality has been stripped of its original context. This might be because recall is lossy, and it’s easier to evangelize soundbites. I will double-click this point in the following paragraphs with three real examples.
...
If we had capitulated to the pressure to blindly follow this “best practice”, the ROI would have been a net negative: the performance claim was false (because the UI we had then wasn’t amenable to the instances where ConstraintLayout is more performant), and we would have had more production defects (because of the complexity of constraints).

Herein lies the rub with the everyday use of the phrase “best practices”: “The best way to do $X$ in $Y$ context” gets lossy compressed into “the best way to do $X$”. Now, people are incurring all the costs of the “best way” without reaping any of its benefits because the context was never relevant to them. The following example is more general.
...
Commonly, you hear this as “premature optimization is the root of all evil” (Knuth’s quote was prematurely optimized). The problem with this shorter variant of the quote is that it’s often applied out of context in the following ways:
1. Performance is often an afterthought because people believe they can optimize later. This is not always true. Some decisions are spread through the codebase and can’t be surgically changed easily. This optimize-later style can also lead to more complexity as optimizations are superimposed into the existing design (e.g., unnecessary threading or caching because individual ops are so slow).
...
3. Failing to distinguish between projects where performance is critical and projects where it isn’t. I shamefully admit that I fall into this camp.

I will close this section with a common mistake with crash-only software. Crash-only software refers to computer programs that handle failures by simply restarting without attempting any sophisticated recovery. Too often, when a team is introduced to this idea, it is applied too broadly. The excerpt below explains this mistake in more detail.
...
Context matters. I have tried to build a case against blindly following “best practices”. However, there are cases where it makes sense to do so. e.g., for legal reasons (collective liability), lack of time to make an informed decision (time crunch), unfamiliar domain, etc.
